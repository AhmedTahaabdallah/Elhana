...
Imports System.Data
Public Class frm_StoreVend_Cdid
    ...
    Public isdafsa As String
    Private Sub frm_StoreVend_Cdid_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
            Dim ut As Boolean
            ut = False
            ...
            rs_Vendors.Open("Select * From Vendores Where Vend_ID=" & vendid_cdid & " And Vend_DelFlag='" & s14 & "'", cn1, ADODB.CursorTypeEnum.adOpenDynamic, ADODB.LockTypeEnum.adLockOptimistic)
            ...
            rs_StoreVend.Open("Select * From Stor Where Stor_DelFlag='" & s & "' And Stor_ID=" & sid_Vend_Cdid & " And Stor_Type='" & order_Type & "'", cn1, ADODB.CursorTypeEnum.adOpenDynamic, ADODB.LockTypeEnum.adLockOptimistic)
    ...
    Private Sub getvendname()
        Try
            ...
            s = False
            Dim stt As String
            stt = "SELECT st.Stor_ID as [رقم الأذن], itm.Item_Name as [أسم الصنف], st.Stor_Date as [تاريخ الأذن], st.Stor_Quntity_No as [الكمية], st.Stor_Price_tin as [السعر], st.Stor_Total_Price as [إجمالى السعر], st.Stor_Payment as [المبلغ المدفوع], st.Stor_Note as [ملاحظات] FROM Stor as st, Itemes as itm, year_monthes as mn where st.Vend_ID=" & vendid_cdid & " And st.Stor_ID_Cdid=" & sid_Vend_Cdid & " And st.Item_ID = itm.Item_ID And st.Stor_DelFlag='" & s & "' And mn.Month_ID=st.Month_ID And mn.Month_DeleFlag='" & s & "' And itm.Item_DelFlag='" & s & "' ORDER BY st.Stor_ID DESC"
    ...
    Private Sub btn_save_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_save.Click
        ...
        Try
            ...
            If rs_Vendors.State = 1 Then rs_Vendors.Close()
            rs_Vendors.Open("Select * From Vendores Where Vend_ID=" & vendid_cdid & " And Vend_DelFlag='" & s & "'", cn1, ADODB.CursorTypeEnum.adOpenDynamic, ADODB.LockTypeEnum.adLockOptimistic)
            If rs_Vendors.EOF Or rs_Vendors.BOF Then

            Else
                val_storintotalprice = rs_Vendors("Vend_Blance_Dein").Value
                isdafsa = rs_Vendors("Vend_Blance_d").Value
            ...
            rs_StoreVend("Vend_ID").Value = vendid_cdid
            ...
            rs_StoreVend("Stor_ID_Cdid").Value = sid_Vend_Cdid
            ...
            ' ''ليه رصيد
            If isdafsa = "defulit" Then

                If val_storintotalprice >= toprice Then
                    Dim ff As Integer
                    ff = val_storintotalprice - toprice
                    If rs_Vendors.State = 1 Then rs_Vendors.Close()
                    rs_Vendors.Open("Select * From Vendores Where Vend_ID=" & vendid_cdid & " And Vend_DelFlag='" & s3 & "'", cn1, ADODB.CursorTypeEnum.adOpenDynamic, ADODB.LockTypeEnum.adLockOptimistic)
                    If rs_Vendors.EOF Or rs_Vendors.BOF Then
            ...
            '    Else
            '        Dim jj As Integer
            '        jj = rs_Vendors("Cust_Blance_Medin").Value - payme
