...
Public Class Export_Money
    ...
    Private Sub Export_Money_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
            Dim ut As Boolean
            ut = False
    ...
    Private Sub btn_save_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_save.Click
        ...
        Try
            ...
            stor_In = rs_Store("Blance_Total").Value

            If stor_In < stor_Out Then
                MsgBox("المبلغ داخل الخزنة لا يكفى لصرف الاذن", MsgBoxStyle.Information, "تنبيه")
                txt_mount.Select()
                Exit Sub
            End If
            ...
            'If ty = "vend" Then
            '    Dim ll As Integer
            '    ll = val_storintotalprice - stor_Out
            '    If ll < z Then
            ...
            If RadioButton3.Checked = True Then
                ...
                Else
                    ...
                    If df = "notdefulit" Then
                        ...
                        End If
                        If tttt <= mo1 Then
                            jj = Val(txt_mount.Text) - rs_Vendors("Cust_Blance_Medin").Value
                            Dim re As String
                            Dim qw As String
                            re = "defulit"
                            qw = "عليه رصيد"
            ...
            n_Type = "Exp"
            If rs_StoreVend.State = 1 Then rs_StoreVend.Close()
            rs_StoreVend.Open("Select* From Imp_Exp_Money Where Money_Type ='" & n_Type & "' And Money_DelFlag='" & s8 & "' And Money_ID = (SELECT MAx(Money_ID)  FROM Imp_Exp_Money)", cn1, ADODB.CursorTypeEnum.adOpenDynamic, ADODB.LockTypeEnum.adLockOptimistic)
    ...
    Private Sub btn_etite_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_etite.Click
        ...
        Try
            ...
            If val_Oldpayment < val_newpayment Then
                ...
                Dim ty1 As Integer
                ty1 = val_Blance_Total2 - val_addtopayment
                If ty1 < z Then
                    MsgBox("رصيد الخزنة يجب الايكون أقل من الصفر", MsgBoxStyle.Information, "تنبيه")
                    txt_mount.Select()
                    Exit Sub
            ...
            If val_Old > val_new Then
                ...
                If rs_StoreVend.EOF Or rs_StoreVend.BOF Then
                    vend_id1 = 0
                Else
                    vend_id1 = rs_StoreVend("Vend_ID").Value
                ...
                'Else
                '    Dim jj As Integer
                '    jj = rs_Vendors("Vend_Blance_Dein").Value + val_addtostore
                '    rs_Vendors("Vend_Blance_Dein").Value = jj
                '    rs_Vendors.Update()
                'End If
                ...
                'If st_id = z Then
                '    If rs_Vendors.State = 1 Then rs_Vendors.Close()
                '    rs_Vendors.Open("Select * From Vendores Where Vend_ID='" & vend_ids & "' And Vend_DelFlag='" & ss1 & "'", cn1, ADODB.CursorTypeEnum.adOpenDynamic, ADODB.LockTypeEnum.adLockOptimistic)
                '    If rs_Vendors.EOF Or rs_Vendors.BOF Then
    ...
    Private Sub Button1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button1.Click
        ...
        Try
            ...
            val_Old = rs_StoreVend("Money_Price").Value
            st_id = rs_StoreVend("Stor_ID").Value
            'vend_ids = rs_StoreVend("Vend_ID").Value
            Dim z As Integer
            z = 0
            If st_id = z Then
                DateTimePicker1.Enabled = True
            ...
            If uioc > oe Then
                ...
                ComboBox3.Text = rs_Vendors("Cust_Name").Value
    ...
    Private Sub btn_delete_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_delete.Click
        ...
        Try
            ...
            If g = vbYes Then
                ...
                'If rs_Vendors.State = 1 Then rs_Vendors.Close()
                'rs_Vendors.Open("Select * From Vendores Where Vend_ID=" & vend_iddelete & " And Vend_DelFlag='" & s3 & "'", cn1, ADODB.CursorTypeEnum.adOpenDynamic, ADODB.LockTypeEnum.adLockOptimistic)
                'If rs_Vendors.EOF Or rs_Vendors.BOF Then

                'Else
                '    Dim jj As Integer
                '    jj = rs_Vendors("Vend_Blance_Dein").Value + ne22
                '    rs_Vendors("Vend_Blance_Dein").Value = jj
                '    rs_Vendors.Update()
                'End If
                ...
                'If st_id = z Then
                '    If rs_Vendors.State = 1 Then rs_Vendors.Close()
                '    rs_Vendors.Open("Select * From Vendores Where Vend_ID='" & vend_ids & "' And Vend_DelFlag='" & s3 & "'", cn1, ADODB.CursorTypeEnum.adOpenDynamic, ADODB.LockTypeEnum.adLockOptimistic)
                '    If rs_Vendors.EOF Or rs_Vendors.BOF Then
    ...
    Private Sub ComboBox3_DropDown(ByVal sender As Object, ByVal e As System.EventArgs) Handles ComboBox3.DropDown
        Try
            ...
            Do While Not rs_Cars.EOF
                ComboBox3.Items.Add(rs_Cars("Cust_Name").Value)
                rs_Cars.MoveNext()
            Loop
            rs_Cars.Close()
            'End If
