...
Public Class Frm_StoreCust
    ...
    Private Sub btn_save_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_save.Click
        ...
        Try
            ...
            Dim s As Boolean
            s = False
            Dim oldmonyblance_print As Integer
            Dim oldcusttpye_print As String = ""
            Dim newmonyblance_print As Integer
            Dim newcusttpye_print As String
            ...
            Else
                ...
                oldmonyblance_print = rs_Vendors("Cust_Blance_Medin").Value
                oldcusttpye_print = rs_Vendors("Cust_Blance_Type").Value
            ...
            'Else
            '    jjb = rs_Vendors("Cust_Blance_Medin").Value
            '    jjss = rs_Vendors("Cust_Blance_Type").Value
            'End If
            'If rs_Vendors.State = 1 Then rs_Vendors.Close()
            'rs_Vendors.Open("Select Max(Fetora_id) From Fetora Where Fetora_custid=" & tt & "", cn1, ADODB.CursorTypeEnum.adOpenDynamic, ADODB.LockTypeEnum.adLockOptimistic)
            'If rs_Vendors.EOF Or rs_Vendors.BOF Then

            'Else
            '    jjd = rs_Vendors("Fetora_id").Value
            'End If

            'If rs_Vendors.State = 1 Then rs_Vendors.Close()
            'rs_Vendors.Open("Select * From Fetora Where Fetora_id=" & jjd & "", cn1, ADODB.CursorTypeEnum.adOpenDynamic, ADODB.LockTypeEnum.adLockOptimistic)
            'rs_Vendors("Fetora_blanceafter").Value = jjb
            'rs_Vendors("Fetora_blanceaftertype").Value = jjss
            'rs_Vendors.Update()
            rs_Vendors.Close()
            ...
            Else
                ...
                newmonyblance_print = rs_Vendors("Cust_Blance_Medin").Value
                newcusttpye_print = rs_Vendors("Cust_Blance_Type").Value
                C_Name = rs_Vendors("Cust_Name").Value

            End If

            Dim g As String
            g = MsgBox("هل تريد طباعة هذا الاذن ؟", MsgBoxStyle.YesNo, "تأكيد الطباعة")

            If g = vbYes Then
                Me.Cursor = Cursors.WaitCursor
                Dim rpt As New Rpt_ShowCust_fatora_save
                Dim frm_rpt As New Frm_rpt_ShowCust_fatora_save
                rdat.Clear()
                'SQLEXPRESS
                ...
                Dim tp As String
                tp = "Cust"
                rcomand = New SqlCommand("Select sm.Stor_ID, (Select vend.Cust_Name From Customers as vend Where vend.Cust_ID =" & tt & ") as custname,(Select vend.Item_Name From Itemes as vend Where vend.Item_ID = sm.Item_ID) as itemname, sm.Stor_Date, sm.Stor_Quntity_No, sm.Stor_Price_tin, sm.Stor_Total_Price, sm.Stor_Payment, sm.Stor_Note From Stor as sm Where sm.Cust_ID =" & tt & " And sm.Stor_ID='" & u_name & "' And sm.Stor_Type='" & tp & "' And sm.Stor_DelFlag='" & ssd & "' ORDER BY sm.Stor_Date ASC", rcon)
                ...
                Dim sy3 As String
                If rs_StoreVend.State = 1 Then rs_StoreVend.Close()
                rs_StoreVend.Open("Select * From Customers Where Cust_ID=" & tt & " And Cust_DelFlag='" & s & "'", cn1, ADODB.CursorTypeEnum.adOpenDynamic, ADODB.LockTypeEnum.adLockOptimistic)
                ...
                rpt.SetParameterValue("T_Name2", oldcusttpye_print)

                rpt.SetParameterValue("C_name", C_Name)
                rpt.SetParameterValue("B_name", oldmonyblance_print)

                rpt.SetParameterValue("T_Name", sy3)
                rpt.SetParameterValue("p_name", ghf)
                ...
                With frm
                    ...
                    .Text = "طباعة فاتورة صادرات لعميل"
    ...
    Private Sub Button1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button1.Click
        ...
        Try
            ...
            If IsDBNull(rs_StoreVend("Fetora_id").Value) Then
                Label32.Text = 0
            Else
                Label32.Text = rs_StoreVend("Fetora_id").Value
            End If
            ...
            If uio = oe Then
                ...
                ComboBox8.Text = rs_Vendors("Vend_Name").Value
    ...
    Private Sub ComboBox8_DropDown(ByVal sender As Object, ByVal e As System.EventArgs) Handles ComboBox8.DropDown
        Try
            ...
            Do While Not rs_Vendors.EOF
                ComboBox8.Items.Add(rs_Vendors("Vend_Name").Value)
                rs_Vendors.MoveNext()
            Loop
            rs_Vendors.Close()
        Catch ex As Exception
    ...
    Private Sub Button2_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button2.Click
        ...
        Try
            ...
            rs_StoreVend.Open("Select* From Stor Where Stor_Type ='" & n_Type & "' And Stor_DelFlag='" & s8 & "' And Stor_ID = (SELECT MAx(Stor_ID) FROM Stor)", cn1, ADODB.CursorTypeEnum.adOpenDynamic, ADODB.LockTypeEnum.adLockOptimistic)
            u_name8 = rs_StoreVend("Stor_ID").Value
            sid_Cust_Cdid = rs_StoreVend("Stor_ID").Value
            Dim mm1 As String
            mm1 = "حفظ صادر"
            s = False
            If rs_StoreVend.State = 1 Then rs_StoreVend.Close()
    ...
    Private Sub Button3_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button3.Click
        ...
        Try
            ...
            If rs_StoreVend.State = 1 Then rs_StoreVend.Close()
            rs_StoreVend.Open("Select Count(Fetora_id) as df From Fetora", cn1, ADODB.CursorTypeEnum.adOpenDynamic, ADODB.LockTypeEnum.adLockOptimistic)
            ...
            Else
                Dim u_name As Integer
                If rs_StoreVend.State = 1 Then rs_StoreVend.Close()
                rs_StoreVend.Open("Select * From Fetora Where Fetora_id= (SELECT MAx(Fetora_id)  FROM Fetora)", cn1, ADODB.CursorTypeEnum.adOpenDynamic, ADODB.LockTypeEnum.adLockOptimistic)
                u_name = rs_StoreVend("Fetora_id").Value
                ssss = u_name + 1
            End If


            If rs_StoreVend.State = 1 Then rs_StoreVend.Close()
            rs_StoreVend.Open("Fetora", cn1, ADODB.CursorTypeEnum.adOpenDynamic, ADODB.LockTypeEnum.adLockOptimistic)
            rs_StoreVend.AddNew()

            rs_StoreVend("Fetora_id").Value = ssss
            rs_StoreVend("Fetora_custid").Value = cust_id
            rs_StoreVend("Fetora_monthid").Value = month_id
            rs_StoreVend("Fetora_date").Value = DateTimePicker1.Value
            rs_StoreVend("Fetora_blancebeforetype").Value = c_type
            rs_StoreVend("Fetora_blancebefore").Value = c_blance
            rs_StoreVend("Fetora_blanceaftertype").Value = c_type
            rs_StoreVend("Fetora_blanceafter").Value = c_blance
            rs_StoreVend("Fetora_totalprice").Value = 0
            rs_StoreVend("Fetora_totalpay").Value = 0
            rs_StoreVend.Update()

            Fetora_id = ssss
            Fetora_custid = cust_id
            Fetora_monthid = month_id
            frm_CustFetora.Show()
        Catch ex As Exception
            MessageBox.Show(ex.Message, "خطأ فى البرنامج")
        End Try
    ...
    Private Sub Button4_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button4.Click
        ...
        Try
            ...
            s = False
            Dim u_name As Integer
            u_name = TextBox2.Text
           
            If rs_StoreVend.State = 1 Then rs_StoreVend.Close()
            rs_StoreVend.Open("Select * From Fetora Where Fetora_id=" & u_name & "", cn1, ADODB.CursorTypeEnum.adOpenDynamic, ADODB.LockTypeEnum.adLockOptimistic)

            If rs_StoreVend.EOF Or rs_StoreVend.BOF Then
                MsgBox("رقم الفاتورة غير موجودة", MsgBoxStyle.Information, "تنبيه")
                TextBox2.Select()
                rs_StoreVend.Close()
                Exit Sub
            End If
            Fetora_id = TextBox2.Text
            Fetora_custid = rs_StoreVend("Fetora_custid").Value
            Fetora_monthid = rs_StoreVend("Fetora_monthid").Value
            fetore_type = "serch"
            TextBox2.Text = ""
            rs_StoreVend.Close()
            frm_CustFetora.Show()
